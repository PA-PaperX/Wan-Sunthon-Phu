...
export default function QuizClient({
  ...
  const handleAnswer = async (word: string) => {
    ...
    const newSession = {
      ...
      score: isCorrect ? latestSessionRef.current.score + 1 : latestSessionRef.current.score,
    ...
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center p-4 sm:p-6 md:p-8 text-[#5C4033] font-sans selection:bg-[#F4D068] selection:text-[#5C4033] overflow-hidden">
          ...
          <div className="flex justify-between items-end mb-2 font-bold text-[#8B5A2B]">
            ...
            <div className="flex flex-col items-end">
              ...
              <span className="text-xl sm:text-2xl text-[#D4A373] drop-shadow-sm flex items-center gap-1">
                <motion.span
                  animate={answered && selectedWord === question.correct ? { scale: [1, 1.5, 1], color: ['#D4A373', '#65B741', '#D4A373'] } : {}}
        ...
        <div className={`grid gap-4 sm:gap-6 w-full ${options.length > 2 ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 md:grid-cols-2'}`}>
          {options.map((word, index) => {
            ...
            if (answered) {
              ...
              if (isCorrect) {
                cardStateClasses = "bg-[#65B741] shadow-[0_8px_0_#458529] z-10 text-white ring-4 ring-[#65B741]/50";
                iconStateClasses = "text-white/30 animate-pulse";
              } else if (isSelected) {
                cardStateClasses = "bg-[#E55B5B] shadow-[0_4px_0_#C23B3B] opacity-95 text-white";
                iconStateClasses = "text-white/20";
              } else {
                cardStateClasses = "bg-gray-300 shadow-[0_4px_0_gray-400] opacity-50 grayscale text-gray-100";
            ...
            return (
              <motion.button
                ...
                whileHover={!answered ? { scale: 1.02, rotateX: 2, rotateY: 2 } : {}}
      ...
      {showExplanation && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 p-4 sm:p-6 backdrop-blur-sm animate-in fade-in">
          <div className="bg-[#FDFBF7] rounded-3xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl flex flex-col items-center relative animate-in zoom-in-95 duration-300">
            <h3 className="text-[#8B5A2B] text-xl sm:text-2xl font-bold mb-2">เฉลย:</h3>
            <h2 className="text-4xl sm:text-5xl font-black text-[#65B741] mb-6 drop-shadow-sm">{question.correct}</h2>
      ...
      <AnimatePresence>
        {answered && !showExplanation && (
          ...
          >
            <h1 className={`text-6xl sm:text-8xl md:text-9xl font-sans font-black drop-shadow-[0_8px_16px_rgba(0,0,0,0.5)] ${
